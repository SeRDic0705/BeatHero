...
- 이동 (방향키) → 1칸 이동
- 공격 키 탭 (누르고 뗌) → 기본 공격 / 1박 차지 릴리즈
- 공격 키 유지 → 차지 시작 또는 차지 유지 (마나 소모 + 배율 누적)
- 공격 키 릴리즈 → 차지 발동

**구간 외 입력:** 무시. 차지 중 키를 구간 외에서 놓으면 차지 취소.
...
### 기본 공격 / 차지 공격 (동일 키 공유)
- **탭** (구간 내 누르고 뗌): 1박 차지 즉시 릴리즈 = 최소 차지 공격
- **유지 후 릴리즈** (2박+): N박 차지 공격

### 차지 메커니즘
- ResponsePhase 입력 구간에서 키 유지 → 마나 1 소모 + 배율 누적
- CallPhase 동안 키 유지 → 차지 상태만 유지 (마나 소모 없음, 배율 누적 없음)
- 입력 구간 내 릴리즈 → 공격 발동 (릴리즈 박자 마나 1 소모)
- 입력 구간 외 릴리즈 → 차지 취소, 해당 박자 마나 미소모, 이전 소모 마나 반환 없음
