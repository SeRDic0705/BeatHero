...
5. 한 작업 단위가 끝나면 진행 상황을 Discord 채널에 보고. **작업 완료 보고는 반드시 터미널이 아니라 Discord 메시지로 보낸다** (사용자는 터미널을 못 봄 — 터미널 출력만으로는 보고로 인정하지 않음).
...
- **기간:** 사용자 해외 출장(약 1주) 중 직접 작업 불가. PC + Unity 에디터는 켜져 있음(MCP 작동 전제).
- **진행:** 이 CLAUDE.md와 `Design/` 기준으로 작업 백로그를 자율 진행. 사용자가 Discord로 특별 요청 시 그것을 **최우선**으로 처리 후 백로그 복귀.
- **전용 채널:** Discord `1513355366908428409` (멘션 불필요 설정). 모든 보고·지시는 이 채널.
- **의사결정:** 막히거나 선택이 필요하면 **터미널 프롬프트가 아니라 Discord 메시지로 질문**한다 (사용자는 터미널을 못 봄). 되돌리기 어렵거나 설계에 영향 주는 결정은 임의 진행하지 말고 Discord로 확인.
- **봇 멘션 규칙:** 채널에 봇이 둘 있음. 특별히 멘션 없으면 클로드가 응답, `<@1513361028971368489>` 멘션 시 그 봇만 응답.
- **보고 단위:** 작업 1건 완료 시 또는 블로커 발생 시 즉시. 장시간 무응답 금지.
- **깃:** git-flow (feature 브랜치 → PR → develop). 모듈 단위로 잘게 커밋, 백로그 1건=1PR. 커밋 전 컴파일 검증 필수. 푸시·PR 생성은 허가됐으나 **직전에 Discord로 알리고 진행** (조용한 자동 푸시 금지). 병합은 사용자가. 상세: `Design/Git_Convention.md`.

---

## 9. 하지 말 것 (Constraints)
