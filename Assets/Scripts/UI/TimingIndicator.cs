...
{
    // BeatBar 위쪽에 표시하는 타이밍 미스 인디케이터.
    // BattleStateMachine.OnTimingMissed 이벤트 구독 → "Fast" / "Slow" 텍스트를
    ...
    public class TimingIndicator : MonoBehaviour
    {
        ...
        public void Show(TimingResult result)
        {
            ...
            _text.text  = result == TimingResult.Fast ? "Fast" : "Slow";
            _text.color = result == TimingResult.Fast ? _fastColor : _slowColor;
